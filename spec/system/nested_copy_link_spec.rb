...
RSpec.describe "Nested view copy link", type: :system do
  ...
  describe "copy link on a nested post" do
    it "copies the nested URL" do
      ...
      expected =
        "#{Discourse.base_url}/nested/#{topic.slug}/#{topic.id}?post_number=#{root_reply.post_number}"
      cdp.clipboard_has_text?(expected)
    end
  end
  ...
  describe "copy link in context=0 view" do
    it "preserves context=0 in the copied URL" do
      ...
      expected =
        "#{Discourse.base_url}/nested/#{topic.slug}/#{topic.id}?post_number=#{child_reply.post_number}&context=0"
      cdp.clipboard_has_text?(expected)
    end
  end
end
