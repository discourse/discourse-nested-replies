...
export default class NestedPostChildren extends Component {
  ...
  get loadMoreLabel() {
    ...
    if (count > 0) {
      return i18n("discourse_nested_replies.load_more_children", { count });
    }
    return i18n("discourse_nested_replies.load_more_children_generic");
  }
  ...
  async loadChildren() {
    ...
    try {
      const data = await ajax(
        `/nested/${this.args.topic.slug}/${this.args.topic.id}/children/${this.args.parentPostNumber}.json?sort=${this.args.sort || "top"}&depth=${this.childDepth}`
      );
  ...
  async loadMore() {
    ...
    try {
      ...
      const data = await ajax(
        `/nested/${this.args.topic.slug}/${this.args.topic.id}/children/${this.args.parentPostNumber}.json?page=${nextPage}&sort=${this.args.sort || "top"}&depth=${this.childDepth}`
      );
